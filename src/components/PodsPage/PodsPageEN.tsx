...
const PodsPage = ({ idp }: GetId) => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`https://rainzhizha.com/api/v1/pods/${idp}/?lang=en`);
    ...
    const [recs, setRecs] = useState<Pods[] | null>(null);
    ...
    return (
        <ProductContainer>
            ...
            <RecContainer>
                ...
                <RecBlock>
                    {recs?.map((item, index) => (
                        <Link key={index} href={{ pathname: '../en/podsproduct', query: { id: item.id } }}>
                            <PodCard
                                id={item.id}
                                code={item.code}
                                title={item.title}
