...
const DetailsPage = ({ idp }: GetId) => {
    ...
    useEffect(() => {
        const fetchProduct = async () => {
            try {
                const response = await axios.get(`https://rainzhizha.com/api/v1/abstracts/${idp}/?lang=en`);
    ...
    return (
        <ProductContainer>
            <LinkPath>
                ...
                <Link href={{ pathname: '/en/category', query: { id: category?.id, title: category?.title } }} >
                    <TextInactive>
                        {category?.title}
                    </TextInactive>
                </Link>
